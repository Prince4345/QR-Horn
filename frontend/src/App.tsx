    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-blue-500/30 overflow-x-hidden relative flex flex-col">
      ...
      <div className="fixed top-4 left-0 right-0 z-50 flex justify-center pointer-events-none">
        <div className="bg-white/5 border border-white/10 backdrop-blur-md p-1 rounded-full flex gap-1 pointer-events-auto">
          <button
            onClick={openScanner}
            className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors flex items-center gap-2 ${view === 'scanner' ? 'bg-white/10 text-white' : 'text-slate-400 hover:text-white'}`}
          >
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors flex items-center gap-2 ${view === 'dashboard' ? 'bg-white/10 text-white' : 'text-slate-400 hover:text-white'}`}
          >
      ...
      <main className="pt-20 pb-12 px-4 min-h-screen flex flex-col items-center">
        ...
        <div className={view === 'dashboard' ? 'w-full flex flex-col items-center' : 'hidden'} aria-hidden={view !== 'dashboard'}>
          <Dashboard onOpenScan={openScan} isActive={view === 'dashboard'} />
        </div>
      </main>
      <IncomingCallModal />
    </div>
