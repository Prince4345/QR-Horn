...
export function parseStickerCustomization(raw: unknown): StickerCustomization {
  ...
  return {
    ...
    headline: (parsed.headline ?? DEFAULT_STICKER_CUSTOMIZATION.headline).trim().slice(0, 40) || DEFAULT_STICKER_CUSTOMIZATION.headline,
    tagline: (parsed.tagline ?? DEFAULT_STICKER_CUSTOMIZATION.tagline).trim().slice(0, 60) || DEFAULT_STICKER_CUSTOMIZATION.tagline,
    ...
    centerLogoImage:
      typeof parsed.centerLogoImage === 'string' && parsed.centerLogoImage.startsWith('data:image/')
        ...
        : null,
    socials,
    qrDark: /^#[0-9a-fA-F]{6}$/.test(parsed.qrDark ?? '') ? parsed.qrDark! : DEFAULT_STICKER_CUSTOMIZATION.qrDark,
    qrLight: /^#[0-9a-fA-F]{6}$/.test(parsed.qrLight ?? '') ? parsed.qrLight! : DEFAULT_STICKER_CUSTOMIZATION.qrLight,
    overlayOpacity:
    ...
    showBadge: typeof parsed.showBadge === 'boolean' ? parsed.showBadge : DEFAULT_STICKER_CUSTOMIZATION.showBadge,
  };
}

export function socialLabel(social: StickerSocial): string {
